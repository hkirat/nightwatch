const {Key, WebElement, WebDriver, Origin, By, until, Condition} = require('selenium-webdriver');
    ...
    return {
      ...
      session: {
        ...
        async clearElementValue(webElementOrId) {
          const element = this.getWebElement(webElementOrId);
          await element.clear();
          // Ref https://github.com/nightwatchjs/nightwatch/issues/4026
          // Chromium seems to not clear the underlying element, and hence we send Backspace explicitly
          const value = await element.getAttribute("value");
          if (value.length) {
            element.sendKeys(new Array(value.length + 1).join(Key.BACK_SPACE))
          }
          return null;
        },
